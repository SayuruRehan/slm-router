...
> Priority: **Critical** (theoretical framing for TriRoute's router)
...
Per our tracker: this is the theoretical foundation we should lean on when justifying action-outcome routing and the quality/cost tradeoffs TriRoute makes. Since the paper's central claim is that estimator quality — not algorithm choice — drives the benefit, this gives us a strong citation for why TriRoute invests specifically in a calibrated multi-head risk estimator rather than a simpler heuristic router. Worth checking during reading: does their formal optimality proof assume binary actions (route/don't-route, escalate/don't-escalate), and if so, does it extend cleanly to TriRoute's three-action setting (accept/repair/regenerate), or is that a genuine theoretical gap we're filling?
